        ...
        <div className="relative mx-auto max-w-7xl flex flex-col items-center text-center">
          ...
          <div className="mt-5 flex flex-wrap gap-2 justify-center">
            {heroChips.map((chip) => (
              <button
                key={chip}
                className="rounded-full border border-white/15 bg-white/10 px-3.5 py-1.5 text-[13px] font-medium text-white/80 transition-colors hover:bg-white/18 hover:text-white"
              >
                {chip}
              </button>
            ))}
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
              {categories.map((cat, index) => {
                ...
                return isFeatured ? (
                  ...
                  <div
                    key={cat.id}
                    className="relative bg-[var(--gray-50)] rounded-2xl px-4 py-6 text-center border-[1.5px] border-[var(--gray-200)] cursor-pointer transition-all duration-250 hover:border-[var(--primary)] hover:-translate-y-1 hover:shadow-[0_8px_24px_rgba(27,79,216,0.13)] hover:bg-white group"
                  >
                    <span className="text-[28px] mb-2.5 block">{cat.icon}</span>
                    <p className="text-[13px] font-semibold text-[var(--navy)] group-hover:text-[var(--primary)] transition-colors mb-1">
                      {cat.name_az}
                    </p>
